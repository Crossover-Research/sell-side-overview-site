...
export function ChartInit() {
  ...
  useEffect(() => {
    // Define initCharts globally if not already
    if (typeof window !== 'undefined' && window.Chart) {
      // Inject and run the chart init script
      const script = document.createElement('script');
      script.textContent = CHART_SCRIPT + '\ninitCharts();';
  ...
  useEffect(() => {
    // Also call after a short delay to handle Chart.js CDN load timing

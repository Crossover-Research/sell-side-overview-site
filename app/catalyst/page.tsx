    ...
    <div onClick={e=>e.target===e.currentTarget&&onClose()} style={{ position:'fixed',inset:0,zIndex:600,background:'rgba(4,9,18,.92)',backdropFilter:'blur(8px)',display:'flex',alignItems:'center',justifyContent:'center',padding:20 }}>
      <div style={{ background:'#0c1a2e',border:'1px solid rgba(255,255,255,.12)',maxWidth:460,width:'100%',position:'relative' }}>
        <div style={{ background:'linear-gradient(135deg,#0f1f38,#162d4a)',padding:'18px 22px',borderBottom:'1px solid rgba(255,255,255,.08)',position:'relative' }}>
          ...
          <p style={{ fontSize:11,color: 'rgba(255,255,255,.58)',margin:0 }}>Same-day if covered &middot; 14-day custom if not</p>
        </div>
            ...
            <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16,flexWrap:'wrap',gap:10 }}>
              ...
              <div style={{ display:'flex',gap:6,alignItems:'center' }}>
                {(['all','active','new','transacted'] as FilterType[]).map(f=>(
                  <button key={f} onClick={()=>setFilter(f)} style={{ padding:'5px 13px',fontSize:11,fontWeight:600,background:filter===f?'rgba(255,255,255,.12)':'transparent',border:'1px solid rgba(255,255,255,.12)',color:filter===f?'rgba(255,255,255,.95)':'rgba(255,255,255,.62)',cursor:'pointer',textTransform:'capitalize' }}>
                ...
                <thead>
                  <tr style={{ borderBottom:'2px solid rgba(255,255,255,.1)' }}>
                    {['Asset','Category','Key Metric','Status',''].map((h,i)=>(
                      <th key={i} style={{ padding:'9px 14px',textAlign:'left',fontSize: 11,fontWeight:700,textTransform:'uppercase',letterSpacing:'.1em',color: 'rgba(255,255,255,.62)',whiteSpace:'nowrap' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {filtered.map((a,i)=>{
                    ...
                    return (
                      <tr key={i} style={{ borderBottom:'1px solid rgba(255,255,255,.05)',cursor:'pointer',transition:'background .12s' }}
                        ...
                        <td style={{ padding:'12px 14px' }}>
                          <button onClick={e=>{e.stopPropagation();isTransacted?setSelectedAsset(a):setRequestOpen(true);}} style={{ background:'rgba(255,255,255,.07)',border:'1px solid rgba(255,255,255,.12)',color: 'rgba(255,255,255,.75)',padding:'5px 12px',fontSize:11,cursor:'pointer',whiteSpace:'nowrap' }}>
